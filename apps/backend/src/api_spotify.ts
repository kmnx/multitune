...
const frontendUrl = port === "80" ? `${host}` : `${host}:${port}`;
  ...
  new SpotifyStrategy(
    {
      ...
      callbackURL: (() => {
        const url = process.env.SPOTIFY_CALLBACK_URL;
        if (!url) {
          throw new Error("SPOTIFY_CALLBACK_URL is not set in environment variables");
        }
        return url;
      })(),
      scope: [
    ...
    async (
      ...
      try {
        ...
        if (!user) {
          ...
          const username =
            profile.displayName || email || `spotify_${profile.id}`;
...
router.get(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      console.log(
        `[Spotify Linked Check] userId: ${userId}, linked: ${isLinked}`
      );
...
router.get(
  ...
  async (req: any, res: Response) => {
    ...
    try {
      ...
      const result = await pool.query(
        "SELECT access_token FROM user_services WHERE user_id = $1 AND service = $2",
        [userId, "spotify"]
      );
...
router.get(
  "/api/db/spotify/playlist/:id/items",
  authenticateJWT,
  async (req: any, res: Response) => {
    const userId = req.user.userId;
