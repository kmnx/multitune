  ...
  async (req: any, res: Response) => {
    try {
      ...
      const result = await pool.query(
        "SELECT 1 FROM user_services WHERE user_id = $1 AND service = $2",
        [userId, "youtube"]
      );
      const isLinked = result.rows.length > 0;
      console.log(
        `[YouTube Linked Check] userId: ${userId}, linked: ${isLinked}`
      );
...
router.get(
  ...
  async (req: any, res: Response) => {
    ...
    async function fetchNewPlaylistsWithToken(
      accessToken: string,
      ...
      do {
        const ytRes: { data: { items: YTPlaylist[]; nextPageToken?: string } } =
        ...
        playlists = playlists.concat(
          ytRes.data.items.filter((pl) => !existingIds.has(pl.id))
        );
    ...
    try {
      console.log(
        `[YouTube Playlists] Querying user_services for userId: ${userId}`
      );
      const result = await pool.query(
        "SELECT access_token, refresh_token FROM user_services WHERE user_id = $1 AND service = $2",
        [userId, "youtube"]
      );
      console.log(`[YouTube Playlists] DB query result:`, result.rows);
      if (!result.rows.length) {
        console.log(
          `[YouTube Playlists] No YouTube link found for userId: ${userId}`
        );
      ...
      const existingPlaylistsRes = await pool.query(
        "SELECT yt_playlist_id, id FROM playlists_youtube WHERE user_id = $1",
        [userId]
      );
      console.log(
        `[YouTube Playlists] Existing playlists DB result:`,
        existingPlaylistsRes.rows
      );
      const existingPlaylistIds = new Set(
        existingPlaylistsRes.rows.map((row: any) => row.yt_playlist_id)
      );
      ...
      try {
        console.log(
          `[YouTube Playlists] Fetching new playlists with accessToken.`
        );
        newPlaylists = await fetchNewPlaylistsWithToken(
          accessToken,
          existingPlaylistIds
        );
      } catch (err: any) {
        if (err.response && err.response.status === 401 && refreshToken) {
          try {
            console.log(
              `[YouTube Playlists] Access token expired, refreshing...`
            );
            const tokenData = await refreshAccessToken(refreshToken);
            accessToken = tokenData.access_token;
            await pool.query(
              "UPDATE user_services SET access_token = $1, updated_at = NOW() WHERE user_id = $2 AND service = $3",
              [accessToken, userId, "youtube"]
            );
            console.log(
              `[YouTube Playlists] Retrying fetch with new accessToken.`
            );
            newPlaylists = await fetchNewPlaylistsWithToken(
              accessToken,
              existingPlaylistIds
            ...
            console.error(
              "[YouTube Playlists] Token refresh failed:",
              refreshErr
            );
            return res
              .status(401)
              .json({
                error:
                  "YouTube authentication expired. Please re-link your account.",
              });
          }
        } else {
          const errorData = err?.response?.data || err?.message || err;
          console.error(
            `[YouTube Playlists] Error for userId: ${userId}:`,
            errorData
          );
          return res
            .status(500)
      ...
      for (const yt_playlist_id of Object.keys(playlistIdMap)) {
        ...
        const existingItemsRes = await pool.query(
          "SELECT yt_video_id FROM playlist_items_youtube WHERE playlist_id = $1",
          [playlist_db_id]
        );
        console.log(
          `[YouTube Playlists] Existing items for playlist ${yt_playlist_id}:`,
          existingItemsRes.rows
        );
        const existingVideoIds = new Set(
          existingItemsRes.rows.map((row: any) => row.yt_video_id)
        );
        const isInitialSync = existingVideoIds.size === 0;
        const part = isInitialSync
        ...
        do {
          const itemsRes: {
            data: { items: YTPlaylistItem[]; nextPageToken?: string };
          } = await axios.get(
        ...
        for (const item of newItems) {
          ...
          const published_at = snippet?.publishedAt
            ...
            : null;
          const channel_title =
            snippet?.videoOwnerChannelTitle || snippet?.channelTitle || null;
          const channel_id =
            snippet?.videoOwnerChannelId || snippet?.channelId || null;
          const thumbnail_url = snippet?.thumbnails?.default?.url || null;
      ...
      console.log(
        `[YouTube Playlists] Returning playlists for userId: ${userId}`
      );
