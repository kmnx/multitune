...
if (!isProd) {
  ...
  backendUrl =
    backendPort === "80" ? `${backendHost}` : `${backendHost}:${backendPort}`;
}
...
const App = () => {
  ...
  const [playlistItemsError, setPlaylistItemsError] = useState<string | null>(
    null
  );
  ...
  const fetchPlaylistItems = async (playlist: any) => {
    ...
    try {
      const res = await fetch(
        `${backendUrl}/auth/api/db/playlist/${playlist.id}/items`,
        {
  ...
  const [selectedSpotifyPlaylist, setSelectedSpotifyPlaylist] = useState<
    any | null
  >(null);
  const [spotifyPlaylistItems, setSpotifyPlaylistItems] = useState<
    any[] | null
  >(null);
  const [spotifyPlaylistItemsLoading, setSpotifyPlaylistItemsLoading] =
    useState(false);
  const [spotifyPlaylistItemsError, setSpotifyPlaylistItemsError] = useState<
    string | null
  >(null);
  ...
  const fetchSpotifyPlaylistItems = async (playlist: any) => {
    ...
    try {
      const res = await fetch(
        `${backendUrl}/auth/api/db/spotify/playlist/${playlist.id}/items`,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        }
      );
  ...
  const handleServiceClick = async (service: Service) => {
    if (service.name === "YouTube") {
      ...
      setServices((svcs) =>
        svcs.map((s) => (s.name === "YouTube" ? { ...s, linked } : s))
      );
      if (!linked) {
    ...
    if (service.name === "Spotify") {
      ...
      setServices((svcs) =>
        svcs.map((s) => (s.name === "Spotify" ? { ...s, linked } : s))
      );
      if (!linked) {
  ...
  useEffect(() => {
    ...
    if (url.searchParams.get("youtube_linked") === "1") {
      ...
      setServices((svcs) =>
        svcs.map((s) => (s.name === "YouTube" ? { ...s, linked: true } : s))
      );
            ...
            <div key={service.name}>
              <div
                ...
                style={{
                  ...
                  padding: "10px 24px",
                  background:
                    selectedService === service.name ? "#393e6e" : "none",
                  cursor: "pointer",
                  borderLeft:
              ...
              {service.name === "YouTube" &&
                ...
                ytSidebarOpen && (
                  ...
                  >
                    {ytLoading && (
                      <div style={{ color: "#fff" }}>Loading playlists...</div>
                    )}
              ...
              {service.name === "Spotify" &&
                ...
                spotifySidebarOpen && (
                  ...
                  >
                    ...
                    {spotifyError && (
                      <div style={{ color: "red" }}>{spotifyError}</div>
                    )}
                            ...
                            <button
                              ...
                              style={{
                                background:
                                  ...
                                  selectedSpotifyPlaylist.id === pl.id
                                    ? "#eebf63"
                                    : "transparent",
                                color:
                                  selectedSpotifyPlaylist &&
                  ...
                  {playlistItems.map((item: any) => (
                    <React.Fragment key={item.yt_video_id}>
                      <tr style={{ borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <td style={{ padding: "6px 12px", color: "#aaa" }}>
                          -
                        </td>
                      </tr>
                      {playingVideoId === item.yt_video_id && (
